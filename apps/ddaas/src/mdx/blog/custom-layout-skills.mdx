---
title: Custom Layout Skills
description: 让AI理解网站的底层Layout组件
date: 2026-05-07
---
...
<Callout title="app/[locale]/layout.tsx" type="info">
  全局壳只保留 i18n 和基础 provider, 不在这里定主题
</Callout>
...
<Callout title="app/[locale]/(home)/layout.tsx" type="info">
  home 组单独定一份主题
</Callout>
...
<Callout title="app/[locale]/(content)/blog/layout.tsx" type="warn">
  blog 组是 `SiteHomeLayout -> SiteDocsLayout` 嵌套, 外层主题生效, 内层 docs 不再接管主题
</Callout>

```tsx
const themeMode = 'dark-only';
...
<Callout title="app/[locale]/(content)/legal/layout.tsx" type="warn">
  legal 组是 `SiteHomeLayout -> SiteDocsLayout` 嵌套, 外层主题生效, 内层 docs 不再接管主题
</Callout>

```tsx
const themeMode = 'light-only';
...
<Callout title="app/[locale]/docs/layout.tsx" type="warn">
  docs 组是单独的 docs 壳, 自己接管主题
</Callout>
...
- 全站统一主题时, 5 个 layout 都用同一个 `mode`
- 多路由组差异主题时, 只改各自最外层 layout 的 `mode`
- `blog / legal` 这种嵌套 docs 场景, `SiteDocsLayout` 里要显式 `themeProvider: false`
- `themeProvider: false` 是 `third-ui` 为嵌套场景准备的内部去重处理, 应用侧一般只需要记住 `themeSwitch.mode`
...
- 全站统一主题就让所有路由组使用同一个 `mode`
- 多路由组差异主题就只在各自最外层 layout 配一次 `mode`
