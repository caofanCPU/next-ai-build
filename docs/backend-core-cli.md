...
2. 生成路由壳：`pnpm dev-scripts backend-core routes:sync --app-dir src/app`  
   ...
   ```
   - 大多数路由是单行 re-export；需要宿主注入配置的路由会生成固定 wrapper。例如 `user/credit-overview` 会注入宿主的 `defaultLocale`、`localePrefixAsNeeded` 和 `next-intl` 翻译 resolver：
   ```ts
   // app/api/user/credit-overview/route.ts
