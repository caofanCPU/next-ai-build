...
应用层不再维护一份跨包 capability 配置。真正能被 bundler 裁剪的边界是静态 import 边界，因此当前模型改为：

- 解析侧需要什么能力，就从 `local-md` 的独立 feature 入口 import 什么能力。
- 渲染侧需要什么能力，就从 `third-ui` 的独立 MDX component feature 入口 import 什么能力。
- 没有被应用 import 的 feature，不应被底层总入口或配置映射提前静态带入。

这意味着能力声明不再放在 `packages/contracts/mdx`，而是由业务接入代码本身表达。`contracts` 只保留跨包稳定业务协议，不再承载 MDX 裁剪配置。

当前正确的依赖方向应该是：

- `local-md` 提供解析基础入口和可选解析 feature 入口。
- `third-ui` 提供渲染基础入口和可选渲染 feature 入口。
- 应用层通过显式 import 组合需要的能力。
- bundler 依据 import 图裁剪未使用的 feature。
...
3. 将应用层原有的 MDX 解析配置迁移为 `local-md` 基础入口加独立 feature 入口，不再直接 import `remark-*`、`rehype-*`、`shiki`。
4. 将应用层原有的 MDX 组件注册逻辑迁移为 `third-ui` 的 site 级基础工厂和独立渲染 feature 入口。
5. 应用层解析侧和渲染侧都通过显式 import 表达能力，不再维护额外 capability 映射文件。
...
9. 确认删除某个 feature import 后，解析链和渲染链会同步收缩，而不是只关闭展示、不关闭底层依赖接入。

## Capability Bundle Matrix

当前能力声明由应用层 import 图决定。
...
应用侧开关位置不是完全一一对应。`code` 和 `math` 同时需要编译侧与渲染侧；`npm` 只影响编译期内容转换；`mermaid` 和 `type-table` 只是 MDX 组件渲染能力。

| 能力 | source 编译侧 | component 渲染侧 |
| --- | --- | --- |
| `base` | `createFumaDocsBaseCompilerOptions()` | `createSiteMdxComponents()` |
| `code` | `createFumaDocsCodeFeature()` | `createCodeMdxComponents()` |
| `math` | `createFumaDocsMathFeature()` | `createMathMdxComponents()` |
| `npm` | `createFumaDocsNpmFeature()` | 不需要 |
| `mermaid` | 不需要 | `createMermaidMdxComponents()` |
| `type-table` | 不需要 | `createTypeTableMdxComponents()` |

### 基础固定能力

即使只 import `base`，当前仍会保留这些基础能力：

- 内容读取、frontmatter/meta schema、TOC、structured data、本地 source loader。
- Markdown/MDX 基础处理：`@mdx-js/mdx`、`remark`、`remark-gfm`、`remark-rehype`、`vfile`、`unified`、`hast-util-to-jsx-runtime`。
- Fuma 基础插件子路径：`remark-heading`、`remark-code-tab`、`remark-structure`、`rehype-toc`。
- Steps 基础语法：`fumadocs-core/mdx-plugins/remark-steps`。当前 steps 被视为基础能力，不单独暴露为 capability。
- 渲染基础组件：`fumadocs-ui/mdx`、tabs、callout、files、accordion、`SiteX`、基础 markdown component map。
- 站点默认轻量组件：`TrophyCard`、`ZiaCard`、`GradientButton`、`ZiaFile`、`ZiaFolder`、`SunoEmbed`。
- 图片展示入口：`ImageZoom` 和 `ImageGrid` 当前在 `base/widgets` 中以 `React.lazy(() => import(...))` 形式接入。它们不在 server feature 主入口顶层静态导入，但如果页面实际渲染图片增强组件，会加载 `react-medium-image-zoom` 和相关图片组件代码。
...
- `rehype-katex`
- `katex`
- `fumadocs-core/mdx-plugins/remark-npm`
- `mermaid`
- `roughjs`
...
| Capability | 解析链加载 | 渲染链加载 | 主要进入产物的底层包 / 模块 | 未 import 时预期 |
| --- | --- | --- | --- | --- |
| `base` | 无额外解析 feature；保留基础 parser、TOC、structure、steps | 基础 MDX 组件、Fuma 基础 UI、站点轻量组件 | `@mdx-js/mdx`、`remark`、`remark-gfm`、`remark-rehype`、`unified`、`vfile`、`fumadocs-core/source`、`fumadocs-core/mdx-plugins/remark-heading`、`remark-code-tab`、`remark-structure`、`rehype-toc`、`remark-steps`、tabs/callout/files/accordion | 基础能力是站点 MDX 的最小接入，不建议关闭 |
| `code` | `local-md/server/features/code` | `third-ui/fuma/server/features/code` | 解析侧：`fumadocs-core/mdx-plugins/rehype-code`、Shiki 相关高亮链路。渲染侧：`fumadocs-ui/components/codeblock` lazy import、语言图标映射 | 不应加载 `rehype-code` / Shiki；代码块退回普通 `pre/code` 基础渲染 |
| `math` | `local-md/server/features/math` | `third-ui/fuma/server/features/math` | 解析侧：`remark-math`、`rehype-katex`。渲染侧：`heavy/math` lazy import，实际包含 `katex` | 不应加载 `remark-math`、`rehype-katex`、`katex` |
| `npm` | `local-md/server/features/npm` | 无 | `fumadocs-core/mdx-plugins/remark-npm`，以及 `remarkInstallOptions` 相关包管理器命令转换逻辑 | 不应加载 `remark-npm`；安装命令增强不生效 |
| `mermaid` | 无 | `third-ui/fuma/server/features/mermaid` | `heavy/mermaid` lazy import；实际渲染时加载 `mermaid`、`roughjs`、watermark/download 相关逻辑 | 不应加载 `mermaid`、`roughjs`；`<Mermaid />` 不注册 |
| `type-table` | 无 | `third-ui/fuma/server/features/type-table` | `fumadocs-ui/components/type-table` lazy import | 不应加载 type-table 组件；`<TypeTable />` 不注册 |

### 裁剪验证方式

验证时不要只看页面功能是否正常，还要看构建产物是否还有不该出现的模块。

推荐按最小 import 图验证：

```ts
// 解析侧只使用 createFumaDocsBaseCompilerOptions()
// 渲染侧只使用 createSiteMdxComponents()，features 为空或不传重型 feature。
```
...
检查 `local-md` preset 产物时，`presets/fuma-docs` 主入口不应静态包含这些字符串：

```bash
rg "rehype-code|remark-math|rehype-katex|remark-npm|shiki" packages/local-md/dist/presets/fuma-docs.js
```

检查 Next 产物时，可以按 feature import 删除后的预期查重型依赖是否仍出现在主构建图中：

```bash
rg "shiki|rehype-code|remark-math|rehype-katex|katex|mermaid|roughjs|remark-npm|type-table" apps/ddaas/.next
```

需要注意：

- ddaas 当前全功能启用时，上述重型依赖出现是正确结果。
- 如果只 import `base`，上述重型依赖不应因为 `local-md` 或 `third-ui` 的顶层静态导入进入产物。
- 如果内容本身或业务额外组件直接 import 了这些包，它们仍可能进入产物；这种情况不属于 capability 层裁剪失效。
- `ImageZoom` / `ImageGrid` 当前属于 base/widgets 体验增强，使用 `React.lazy` 延迟加载；如果要把图片增强也做成独立 capability，需要新增类似 `image` 的 capability。
